...
import { markNotificationReadAction, deleteNotificationAction, deleteAllReadNotificationsAction } from '@/lib/actions';
...
export function NotificationsPopover() {
  ...
  const handleMarkAllRead = async () => {
    ...
    for (const id of unreadIds) {
        await markNotificationReadAction(user.uid, id, isAdmin);
    }
  ...
  const handleDeleteAllRead = async () => {
    ...
    const result = await deleteAllReadNotificationsAction(user.uid, isAdmin);
    if(result.success) {
  ...
  const handleNotificationClick = async (notif: Notification) => {
    if (!user) return;
    if (!notif.read) {
        await markNotificationReadAction(user.uid, notif.id, isAdmin);
    }
    if (notif.link) {
  ...
  const handleDelete = async (e: React.MouseEvent, notificationId: string) => {
    ...
    if (!user) return;
    await deleteNotificationAction(user.uid, notificationId, isAdmin);
  }
